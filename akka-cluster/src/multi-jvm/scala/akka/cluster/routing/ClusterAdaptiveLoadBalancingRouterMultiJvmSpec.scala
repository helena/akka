...
with ImplicitSender with DefaultTimeout {
  ...
  lazy val router3 = system.actorOf(Props[SomeActor].withRouter(ClusterRouterConfig(ClusterAdaptiveMemoryLoadBalancingRouter(),
    ClusterRouterSettings(totalInstances = 3, maxInstancesPerNode = 1))), "router3")
  lazy val router4 = system.actorOf(Props[SomeActor].withRouter(ClusterRouterConfig(ClusterAdaptiveMemoryLoadBalancingRouter(),
    ClusterRouterSettings(totalInstances = 3, maxInstancesPerNode = 1))), "router4")
  lazy val router5 = system.actorOf(Props[SomeActor].withRouter(ClusterRouterConfig(ClusterAdaptiveMemoryLoadBalancingRouter(),
